...
if 'stop' and 'converted' not in st.session_state:
...
uploaded_file = st.file_uploader(language.languages[st.session_state.lang_state]['upload_song'], type=["mp3", "wav"], accept_multiple_files=False)
st.session_state['stop'] = False
st.session_state['converted'] = False
...
if uploaded_file is not None:
    ...
    if uploaded_file is not None:
        ...
        with tempfile.NamedTemporaryFile(delete=False) as temp_file:
            ...
            temp_file_path = os.path.join("./tempDir/", temp_file.name)

        # st.write(f"File saved to temporary location: {temp_file_path}")

        # Optionally, you can delete the file after processing
        st.audio(temp_file_path)
        if st.session_state.converted != True:
            ...
            if st.button(language.languages[st.session_state.lang_state]['button_play'], use_container_width=True):
                ...
                print("predict")
                model_output, midi_data, note_events = predict(f'{temp_file_path}', model)
                print("predict done")
                note_events = [(round(item[0], 4), round(item[1], 4), item[2], round(item[3] * 32767)) for item in
                               note_events]
                ...
                base_note = [item for item in transposed if
                             ((item[2] - nada_dasar <= 24 and item[2] - nada_dasar >= 0))]
                ...
                fix_duration = [(item[0], item[1], 1 if item[1] - item[0] <= 0 else item[2], item[3]) for item in
                                fix_duration]
